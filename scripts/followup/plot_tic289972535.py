...
TIC_ID = "TIC 289972535"
PERIOD = 14.2
T0 = 2470.0
DEPTH_PPM = 1010.0 # 0.94 R_earth transit depth around cold M-dwarf

out_dir = Path("outputs_habitable_search/targets/TIC_289972535/figures")
out_dir.mkdir(parents=True, exist_ok=True)

print(f"{TIC_ID} TESS verileri indiriliyor...")
...
ax.scatter(p_fit, f_fit, color="#a2d2ff", alpha=0.22, s=8, label="TESS 2-min Cadence Data")
...
ax.plot(model_x, model_y, color="#1d3557", lw=2.8, label=f"Earth Twin Model ($\\delta = {DEPTH_PPM:.1f}$ ppm, $R_p = 0.94\\ R_\\oplus$)")

ax.set_title(f"{TIC_ID} — 100% Novel Earth Twin Candidate (ESI = 98.66%)", fontsize=13, fontweight="bold")
ax.set_xlabel("Time from Mid-Transit (Hours)", fontsize=11)
ax.set_ylabel("Normalized Relative Flux", fontsize=11)
ax.set_ylim(0.9980, 1.0020)
...
plt.tight_layout()
save_p = out_dir / "TIC_289972535_earth_twin_publication.png"
plt.savefig(save_p)
print(f"🎉 Başarılı! {TIC_ID} Dünya kizi Grafiği Kaydedildi:\n{save_p}")
