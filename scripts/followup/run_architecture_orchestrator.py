...
def main():
    ...
    for _, row in tqdm(targets.iterrows(), total=len(targets)):
        ...
        inp = DiscriminatorInput(
            target_id=f"TIC {tic}",
            sector=sector,
            primary_depth_ppm=p_dep,
            primary_sig=p_sig,
            secondary_depth_ppm=s_dep,
            secondary_sig=s_sig,
            l4_depth_ppm=l4_dep,
            l4_sig=l4_sig,
            l5_depth_ppm=l5_dep,
        ...
        out = {
            ...
            "Primary_Sig": round(p_sig, 1),
            "Secondary_Sig": round(s_sig, 1),
            "L4_Sig": round(l4_sig, 1),
            "L5_Sig": round(l5_sig, 1),
            "Odd_Even": round(odd_even, 1),
            "EB_Score": res.eb_score,
            "Co_Score": res.coorbital_score,
            "Verdict": res.verdict,
